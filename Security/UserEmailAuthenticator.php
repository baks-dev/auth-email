...
final class UserEmailAuthenticator extends AbstractAuthenticator
{
		...
		return new SelfValidatingPassport(
			new UserBadge($LoginDTO->getEmail(), function() use ($LoginDTO, $form){
				if($form->isSubmitted() && $form->isValid())
				{
					...
					$account = $this->accountEventActiveByEmail->get($LoginDTO->getEmail());
					if($account === null)
					{
						return null;
					}
					...
					$cache->delete('group-'.$account->getUser()->getValue());
					
					return $this->userById->get($account->getUser());
				}
	...
	public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $firewallName) : ?Response
	{
		...
		if($request->isXmlHttpRequest())
		{
			...
			return new JsonResponse(['success'], 302);
		}
